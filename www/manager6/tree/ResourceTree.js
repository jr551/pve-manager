...
Ext.define('PVE.tree.ResourceTree', {
	...
	initComponent: function () {
		...
		let updateTree = function () {
			...
			// Add dummy Docker container nodes
			// Add dummy Docker container nodes with URLs
			let dockerContainers = [
				{ id: 'docker1', type: 'docker', text: 'Docker Container 1', url: 'http://example.com/docker1', iconCls: 'fa fa-docker fa-play-circle' },
				{ id: 'docker2', type: 'docker', text: 'Docker Container 2', url: 'http://example.com/docker2', iconCls: 'fa fa-docker fa-stop-circle' },
			];

			dockerContainers.forEach(function (info) {
				let child = me.groupChild(rootnode, info, groups, 0);
				if (child) {
					index[info.id] = child;
				}
			...
			// Add a click event listener to handle navigation
			me.on('itemclick', function (view, record, item, index, event) {
				if (record.data.type === 'docker' && record.data.url) {
